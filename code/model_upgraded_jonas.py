...
def generate_price_grid(grid_size, prices, probs_prices):
    ...
    # increase prices near center
    for i in range(grid_size):
        for j in range(grid_size):
            s = grid_size/2
            dist_center = np.sqrt(((abs(i-s)/s)**2 + (abs(j-s)/s)**2)/2)
            grid_prices[i][j] += round(1 - dist_center, 1) * 6000 # rounded so similar prices are still grouped by find_empty_location
...
for i in range(grid_size):
    for j in range(grid_size):
        sat_avg += calculate_total_satisfaction(grid, i, j, agent_weights)
sat_avg /= grid_size**2
print(f'avg satisfaction before: {round(sat_avg, 3)}')
...
for i in range(grid_size):
    for j in range(grid_size):
        sat_avg += calculate_total_satisfaction(grid, i, j, agent_weights)
sat_avg /= grid_size**2
print(f'avg satisfaction after: {round(sat_avg, 3)}')

# Visualize the final state of the grid after simulation
plt.figure()
plt.title('Price Grid')
plt.imshow(grid_prices, cmap='cool', vmin=0, vmax=10000)
